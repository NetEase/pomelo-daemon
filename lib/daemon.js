...
daemon.prototype.insertMessage = function(filename, msg, cb) {
	...
	if (start < 0) {
		logger.error('error log msg format without json strings %j', msg);
		cb(null);
		return;
	}
	...
	mongoClient.insert(JSON.parse(msg), function(err, results) {
		...
		if (results && results.length) {
			...
			if(process.env.debug)
				logger.info('file %j %j', filename, num);
		}
		cb(null);
	});
};
...
daemon.prototype.addTailFile = function(filepath, filename) {
	...
	var tail = new Tail(filepath, eol);
...
var syncToMongo = function(dir, files, self, cb) {
	...
	async.whilst(
		...
		function(callback) {
			...
			var count = parseInt(getReadLine(filename));
...
var syncFileToMongo = function(filepath, filename, from, self, cb) {
	...
	pipe.on('data', function(data) {
		pipe.pause();
		if (process.env.debug)
			logger.info('start sync file %j from: %j now: %j', filename, from ,now);
		if (now <= from || from === 0) {
			pipe.resume();
			now++;
		} else {
